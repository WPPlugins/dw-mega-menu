<?php
add_action( 'wp_update_nav_menu_item', 'dw_megamenu_custom_nav_menu', 10, 3 );
function dw_megamenu_custom_nav_menu( $menu_id, $menu_item_db_id, $args ) {

	$classes = array(
		'menu-item-enable-megamenu'     => '_dw_megamenu_enable',
		'menu-item-hide-label'          => '_dw_megamenu_hide_label',
		'menu-item-enable-row'          => '_dw_megamenu_enable_row',
		'menu-item-widgets'             => '_dw_megamenu_widget',
		'menu-item-enable-megamenu-tab' => '_dw_megamenu_tab_enable',
		'menu-item-enable-fullwidth'    => '_dw_megamenu_enable_fullwidth',
	);
...
add_filter( 'wp_edit_nav_menu_walker', 'dw_megamenu_custom_nav_edit_walker', 10, 2 );
function dw_megamenu_custom_nav_edit_walker($walker,$menu_id) {
	return 'DW_Megamenu_Menu_Items';
}

class DW_Megamenu_Menu_Items extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$megamenu_icon              = get_post_meta( $item->ID, '_dw_megamenu_sicon', true );
		$megamenu_enable_row        = get_post_meta( $item->ID, '_dw_megamenu_enable_row', true );
		$megamenu_hide_label        = get_post_meta( $item->ID, '_dw_megamenu_hide_label', true );
		$megamenu_widget            = get_post_meta( $item->ID, '_dw_megamenu_widget', true );
		$megamenu_tab_enable        = get_post_meta( $item->ID, '_dw_megamenu_tab_enable', true );
		$megamenu_enable_fullwidth  = get_post_meta( $item->ID, '_dw_megamenu_enable_fullwidth', true );
			...
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						<span class="item-type show-if-mega-menu-top"><?php echo __('Mega Menu', 'dw-megamenu') ?></span>
						<span class="item-type show-if-mega-menu-column"><?php echo __('Column', 'dw-megamenu') ?></span>
						<span class="item-type show-if-mega-menu-tab"><?php echo __('Tab', 'dw-megamenu') ?></span>
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
				...
				<p class="description description-wide <?php if ( $item->type == 'menu_widget' ) echo 'hide-if-menu-is-widget' ?>">
					<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<span class="show-if-mega-menu-top show-if-mega-menu-top-tab"><?php _e( 'Mega Menu Label', 'dw-megamenu' ) ?></span>
						<span class="show-if-mega-menu-column"><?php _e( 'Mega Menu Column Label', 'dw-megamenu' ) ?></span>
						<span class="show-if-mega-menu-tab"><?php _e( 'Mega Menu Tab Label', 'dw-megamenu' ) ?></span>
						<span class="hide-if-mega-menu-top-tab hide-if-mega-menu-top hide-if-mega-menu-column hide-if-mega-menu-tab"><?php _e( 'Navigation Label', 'dw-megamenu' ) ?></span>
						<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title dw-mega-menu-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>

					<label class="dw-mega-menu-title-off-label" for="edit-menu-item-hide-label-<?php echo $item_id; ?>">
						<input type="checkbox" class="dw-mega-menu-disable-label" name="menu-item-hide-label[<?php echo $item_id; ?>]" id="edit-menu-item-hide-label-<?php echo $item_id; ?>" <?php checked( $megamenu_hide_label, 'true' ); ?> />
						<?php _e( 'Hide', 'dw-megamenu' ); ?>
					</label>
				</p>
				...
				*/
				...
				<p class="field-enabel-row description description-wide hide-if-mega-menu-tab">
					<label for="edit-menu-item-enable-row-<?php echo $item_id; ?>">
						...
						<?php _e( 'Start a new row', 'dw-megamenu' ); ?>
					</label>
				</p>
				...
				*/
				...
				<p class="field-sicon description description-wide <?php if ( $item->type == 'menu_widget' ) echo 'hide-if-menu-is-widget' ?>">
					<label for="edit-menu-item-sicon-<?php echo $item_id; ?>">
						<?php _e( 'Icon', 'dw-megamenu' ); ?><br>
						<!-- <button class="button dw_megamenu_select_sicon_menu" data-id="<?php echo $item_id; ?>" ><?php _e( 'Select Icon', 'dw-megamenu' ); ?></button> -->
						<a href="#" class="button dw_megamenu_select_sicon_menu" data-id="<?php echo $item_id; ?>" data-action="mega-menu-pick-icon">
							<span class="inline-if-empty"><?php _e('Add Icon', 'dw-megamenu') ?></span>
						</a>&nbsp;
						<span class="dw-megamenu-icon-prev-<?php echo $item_id; ?> <?php if ( empty( $megamenu_icon ) ) { echo "hide-if-icon-empty"; } ?>">
							<i class="<?php echo $megamenu_icon; ?>"></i>
							<a class="dw-remove-icon-style remove_icon" data-id="<?php echo $item_id; ?>" title="Remove Icon">
				...
				<p class="field-megamenu description description-wide <?php if ( $item->type == 'menu_widget' ) echo 'hide-if-menu-is-widget' ?>">
					<label for="edit-menu-item-enable-megamenu-<?php echo $item_id; ?>">
						<input type="checkbox" class="dw-megamenu-enable" name="menu-item-enable-megamenu[<?php echo $item_id; ?>]" id="edit-menu-item-enable-megamenu-<?php echo $item_id; ?>" <?php checked( $megamenu_enable, 'true' ); ?> />
						<?php _e( 'Use as Mega Menu', 'dw-megamenu' ); ?>
					</label>
				</p>
				...
				<p class="field-enabel-fullwidth description description-wide">
					<label for="edit-menu-item-enable-fullwidth-<?php echo $item_id; ?>">
						...
						<?php _e( 'Full Width Drop Down', 'dw-megamenu' ); ?>
					</label>
				</p>
				...
				<p class="field-megamenu-type description description-wide <?php if ( $item->type == 'menu_widget' ) echo 'hide-if-menu-is-widget' ?>">
					<label for="edit-menu-item-megamenu-type-<?php echo $item_id ?>">
						<?php _e( 'Use Submenus as:', 'dw-megamenu' ); ?>
						<select class="widefat dw-megamenu-type" name="menu-item-megamenu-type[<?php echo $item_id; ?>]" id="edit-menu-item-megamenu-type-<?php echo $item_id ?>">
							<option <?php selected( $megamenu_type, 'column' ) ?> value="column"><?php _e( 'Column', 'dw-megamenu' ); ?></option>
							<option <?php selected( $megamenu_type, 'tab' ) ?> value="tab"><?php _e( 'Tab', 'dw-megamenu' ); ?></option>
						</select>
					</label>
				</p>
