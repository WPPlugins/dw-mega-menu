<?php
class DW_Megamenu_Widgets
{

	public $prefix = 'dw_megamenu_widget';
	...
	public function add_meta_box() {
		add_meta_box( 'dw-menu-widgets', __( 'Widgets', 'dw-megamenu' ), array( $this, 'metabox' ), 'nav-menus', 'side', 'default' );
	}
	...
	public function metabox() {
		...
		if ( empty( $wp_registered_sidebars[$this->prefix] ) || empty( $sidebars_widgets[$this->prefix] ) || !is_array( $sidebars_widgets[$this->prefix] ) ) {
			...
			$no_widget_available .= sprintf( __( '<a href="%s">Please add a widget</a> to the <em>Widgets in Menu</em> area', 'dw-megamenu' ), admin_url( 'widgets.php' ) );
			$no_widget_available .= '</p>';
		...
		?>
		<div class="dw_megamenu_widget" id="dw_megamenu_widget">
			<?php echo $output; ?>
			<p class="button-controls">
				<span class="add-to-menu">
					<input type="submit"<?php wp_nav_menu_disabled_check($nav_menu_selected_id); ?> class="button-secondary submit-add-to-menu right" value="<?php _e( 'Add to menu', 'dw-megamenu' ); ?>" name="dw-megamenu-menu-item" id="submit-megamenu-widget" />
	...
	public function register_sidebar_for_widget_menu() {
		...
		$args = array(
			'name'                  => __( 'Widgets in Menu', 'dw-megamenu' ),
			'id'                        => $this->prefix,
			'description'   => 'Add a widget into Megamenu',
			'class'                 => '',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '<h3 class="dw-megamenu-widget-title">',
			'after_title'   => '</h3>',
		);
	...
	public function start_el( $item_output, $item, $depth, $args ) {
		...
		$dw_megamenu_widget = $wp_registered_widgets[$id];
...
return new DW_Megamenu_Widgets();