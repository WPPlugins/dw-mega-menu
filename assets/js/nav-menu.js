jQuery(document).ready(function($){
	...
	// add icon
	$(document).on( 'click', '.dw_megamenu_select_sicon_menu', function(e) {
		e.preventDefault();

		var $this = $(this),
			$id = $this.data('id'),
			$prev = $( 'span.dw-megamenu-icon-prev-' + $id ).find( 'i' );
		...
		$( '#button_select_icon' ).on('click', function() {
			...
			$('span.dw-megamenu-icon-prev-' + $id + ' i').attr( 'class', '' );
			$('span.dw-megamenu-icon-prev-' + $id + ' i').addClass( $checked );
			$('span.dw-megamenu-icon-prev-' + $id ).removeClass('hide-if-icon-empty');
	...
	$(document).on( 'click', '.remove_icon', function(e) {
		...
		var $this = $(this),
				$id = $this.data('id');
		$( 'span.dw-megamenu-icon-prev-' + $id ).removeClass('dw-remove-icon-style').addClass('hide-if-icon-empty');
		$( 'span.dw-megamenu-icon-prev-' + $id + ' i' ).attr( 'class', '' );
	...
	$(document).on( 'change', '.menu-item-depth-0 input.dw-megamenu-enable', function() {
	...
	$(document).on('change', '.menu-item-depth-0 select.dw-megamenu-type', function() {
		var t = $(this);
		$(this).each(function() {
			if ( t.val() == 'tab' ) {
				menu.children( '.menu-item-depth-0:has( select.dw-megamenu-type )' ).each( function() {
					var item = $(this);
	...
	if ( $( '.menu-item-depth-0 input.dw-megamenu-enable:checked' ) ) {
		$(this).parent().addClass( 'mega-menu' );
	}

	if ( $( '.menu-item-depth-0 input.dw-megamenu-tab-enable:checked' ) ) {
		$(this).parent().addClass( 'mega-menu-tab' );
	}

	function update() {
		menu.children().removeClass('mega-menu');
		menu.children('.menu-item-depth-0:has(.dw-megamenu-enable:checked)').each(function () {
			var item = $(this);
	...
	function update_tab() {
		var t = $('.menu-item-depth-0 select.dw-megamenu-type');
		menu.children().removeClass('mega-menu-tab');
		menu.children().removeClass('mega-menu-column');
		t.each(function() {
			if ( $(this).closest('li').find( '.dw-megamenu-enable').is(':checked') ) {
				if ( $(this).val() == 'tab' ) {
					menu.children( '.menu-item-depth-0:has( select.dw-megamenu-type )' ).each( function() {
						var item = $(this);
						if ( item.find( 'select.dw-megamenu-type' ).val() == 'tab' ) {
							item.removeClass( 'mega-menu-column' ).addClass( 'mega-menu-tab' );
							item.nextUntil('.menu-item-depth-0').removeClass('mega-menu-column').addClass( 'mega-menu-tab' );
						}
					...
					menu.children( '.menu-item-depth-0:has( select.dw-megamenu-type )' ).each( function() {
						var item = $(this);
						if ( item.find( 'select.dw-megamenu-type' ).val() == 'column' ) {
							item.addClass( 'mega-menu-column' ).removeClass( 'mega-menu-tab' );
							item.nextUntil('.menu-item-depth-0').addClass('mega-menu-column').removeClass( 'mega-menu-tab' );
						}
					});
	...
	$(document).on('change', '.menu-item-depth-0 .dw-megamenu-enable', update);
	$(document).on('change', '.menu-item-depth-0 select.dw-megamenu-type', update_tab);
	...
	$('#submit-megamenu-widget').on('click', function(e) {
		console.log( 'a' );
		dw_megamenu_add_item();
	});

	function dw_megamenu_add_item() {
		if ( 0 == $('#menu-to-edit').length ) {
			return false;
		}

		var t = $( '.dw_megamenu_widget' ), menuItems = {}, checkboxs = t.find( 'li input[type="checkbox"]:checked' ), re = /menu-item\[([^\]]*)/;
	...
	$( 'button#submit-save-setting' ).on( 'click', function(e) {
		...
		$.ajax({
			...
			dataType: 'json',
			data: {
				action: 'dw_mega_menu_save_option',
